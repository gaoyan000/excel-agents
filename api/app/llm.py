...
def _nl_sql_system(schema_desc: str) -> str:
    ...
    return (
        ...
        "  --   default polish (use this shape unless the user asks otherwise):\n"
        "  --     1) source_file -> just the company prefix via REGEXP_EXTRACT\n"
        "  --     2) skip rows whose date does not parse (filter in CTE) so\n"
        "  --        you never get a stray '月' column from CONCAT(NULL,'月')\n"
        "  --     3) USING COALESCE(ROUND(SUM(...), 2), 0)  -> no NULL, no\n"
        "  --        float-precision noise like 1149.39999999\n"
        "  --     4) suffix the pivoted column name with the value-column's\n"
        "  --        common name, e.g. '月总运费' / '月销量' / '月_revenue'\n"
        "  WITH base AS (\n"
        "    SELECT REGEXP_EXTRACT(source_file, '^[^0-9.]+', 0) AS \"数据来源\",\n"
        "           \"客户\",\n"
        "           month(TRY_CAST(\"日期\" AS DATE)) AS m,\n"
        "           TRY_CAST(\"运费合计\" AS DOUBLE) AS amt\n"
        "    FROM t\n"
        ...
        "    PIVOT base\n"
        "      ON CONCAT(m, '月总运费')\n"
        "      USING COALESCE(ROUND(SUM(amt), 2), 0)\n"
        "      GROUP BY \"数据来源\", \"客户\"\n"
        "  ) ORDER BY \"数据来源\", \"客户\"\n\n"
        "Schema: " + schema_desc + "\n\n"
